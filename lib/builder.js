...
module.exports = class Builder {
  ...
  build() {
    ...
    return promise
      ...
      .then(nodeWrapper => {
        this.buildHeimdallTree(nodeWrapper);
        return nodeWrapper;
      })
      .finally(() => {
  ...
  setupHeimdall() {
    this.on('beginNode', node => {
      const name = node instanceof SourceNodeWrapper ? node.nodeInfo.sourceDirectory : node.label;
      node.__heimdall__ = heimdall.start({
        name,
        label: node.label,
        broccoliNode: true,
        broccoliId: node.id,
    ...
    this.on('endNode', node => {
      if (node.__heimdall__) {
        node.__heimdall__.stop();
      }
    });
  }

  buildHeimdallTree(outputNodeWrapper) {
    const heimdallRootNode = heimdall.root;
    const heimdallByBroccoliId = {};

    // Index heimdall nodes by ID and remove from the graph (re-add below)
    const toRemove = [];
    heimdallRootNode.forEachChild(child => {
    ...
    // If there are no nodes, return now
    if (toRemove.length === 0) {
      return;
    }

    // If the outputNodeWrapper node could not be found, then we didn't finish the build
    if (!heimdallByBroccoliId[outputNodeWrapper.id]) {
      return;
    }
    ...
    const traverseTree = function traverseTree(nodeWrapper, heimdallNode) {
      heimdallNode.stats.time.duration = heimdallNode.stats.time.self;
      ...
      for (let inputNodeWrapper of nodeWrapper.inputNodeWrappers) {
        ...
        if (childHeimdallNode.parent) {
          const cookie = heimdall.start(Object.assign({}, childHeimdallNode.id));
          cookie.stop();
          childHeimdallNode = cookie._node;
          childHeimdallNode.id.broccoliCachedNode = true;

          // We must remove the newly created node so it can be re-added below
          childHeimdallNode.remove();
        }

        heimdallNode.addChild(childHeimdallNode);
        heimdallNode.stats.time.self += traverseTree(inputNodeWrapper, childHeimdallNode);
      }

      return heimdallNode.stats.time.self;
    };

    const time = traverseTree(outputNodeWrapper, heimdallByBroccoliId[outputNodeWrapper.id]);
    heimdallRootNode.addChild(heimdallByBroccoliId[outputNodeWrapper.id]);
    heimdallRootNode.stats.time.duration = heimdallRootNode.stats.time.self;
    heimdallRootNode.stats.time.self += time;
  }
